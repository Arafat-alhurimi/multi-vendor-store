...
class PromotionItemsRelationManager extends RelationManager
{
    ...
    protected static ?string $title = 'المشاركون وطلبات الانضمام';
    ...
    {
        return [
            'participants' => Tab::make('المشاركون بالعرض')
                ->badge((string) $this->getOwnerRecord()->items()->where('status', 'approved')->count())
                ->badgeColor('success')
                ->modifyQueryUsing(fn (Builder $query): Builder => $query->where('status', 'approved')),
            'join_requests' => Tab::make('طلبات الانضمام')
                ->badge((string) $this->getOwnerRecord()->items()->where('status', 'pending')->count())
                ->badgeColor('warning')
    ...
    private function resolveRequestSummary(PromotionItem $item): array
    {
        ...
        return [
            ...
            'cart_additions_count' => $isParticipant ? $this->countBenefitedCartAdditions($productIds) : 0,
            'orders_count' => $isParticipant ? $this->countBenefitedOrders($productIds) : 0,
        ];
    }
    ...
    private function countBenefitedCartAdditions(Collection $productIds): int
    {
        if ($productIds->isEmpty()) {
            return 0;
        }
    ...
    private function countBenefitedOrders(Collection $productIds): int
    {
        if ($productIds->isEmpty()) {
            return 0;
        }
